...
context('Testing samesource_C (Rcpp)')
...
set.seed(seed)
X <- rwish_arma(df, Sigma, FALSE, FALSE)
X.inv <- solve(X)
X.chol <- chol(X)
X.inv.chol <- chol(X.inv)


test_that('rwish_arma is correct: pass Cholesky, return full', {
	set.seed(seed)
	X.test <- rwish_arma(df, Sigma.chol, TRUE, FALSE)
	expect_equal(X, X.test)
	expect_equal(chol(X), chol(X.test))
})

test_that('rwish_arma is correct: pass Cholesky, return Cholesky', {
	set.seed(seed)
	X.chol.test <- rwish_arma(df, Sigma.chol, TRUE, TRUE)
	expect_equal(X, t(X.chol.test) %*% X.chol.test)
	expect_equal(X.chol, X.chol.test)
})

test_that('rwish_arma is correct: pass full, return Cholesky', {
	set.seed(seed)
	X.chol.test <- rwish_arma(df, Sigma, FALSE, TRUE)
	expect_equal(X, t(X.chol.test) %*% X.chol.test)
	expect_equal(X.chol, X.chol.test)
})
...
# rwish_arma should generate from the same distribution as rWishart does
...
list.rwish_arma.samples <- replicate(n.samples, rwish_arma(df.Anderson, Sigma), simplify = FALSE)

rWishart.mean <- apply(rWishart.samples, c(1,2), mean)
rwish_arma.mean <- apply(simplify2array(list.rwish_arma.samples), c(1,2), mean)

# wishart.mean.exact
# rWishart.mean

# stopifnot(all.equal(rWishart.mean, wishart.mean.exact, tolerance = 1e-2))
# stopifnot(all.equal(rwish_arma.mean, wishart.mean.exact, tolerance = 1e-2))
...
is.upper <- (rwish_arma.mean + z * sd.mc.error) <  wishart.mean.exact
is.lower <- (rwish_arma.mean - z * sd.mc.error) >  wishart.mean.exact
test.result <- all(is.upper & is.lower)

test_that(paste('rwish_arma converges to the mean, confidence', 1 - alpha),
...
dens.iwishart <- diwishart_inverse(X.inv, df.Press, Sigma,            log  = FALSE, is.chol = FALSE)
dens.iwishart.arma <- diwishart_inverse_arma(X.inv, df.Press, Sigma,  logd = FALSE, is_chol = FALSE)
ldens.iwishart.arma <- diwishart_inverse_arma(X.inv, df.Press, Sigma, logd = TRUE,  is_chol = FALSE)
...
# Cholesky passes
ldens.iwishart.arma.chol <- diwishart_inverse_arma(X.inv.chol, df.Press, Sigma.chol, logd = TRUE,  is_chol = TRUE)
dens.iwishart.chol <- diwishart_inverse(X.inv.chol, df.Press, Sigma.chol,            log  = FALSE, is.chol = TRUE)
dens.iwishart.arma.chol <- diwishart_inverse_arma(X.inv.chol, df.Press, Sigma.chol,  logd = FALSE, is_chol = TRUE)

test_that('R diwishart_inverse (Press) = Rcpp diwishart_inverse_arma (Press), pass Cholesky', {
...
test_that('Rcpp diwishart_inverse_arma (Press), pass full = Rcpp diwishart_inverse_arma (Press), pass Cholesky', {
...
dens.iwishart.inverse <- diwishart_inverse(X.inv, df.Press, Sigma,           log  = FALSE, is.chol = FALSE)
dens.iwishart.inverse.arma <- diwishart_inverse_arma(X.inv, df.Press, Sigma, logd = FALSE, is_chol = FALSE)
...
dens.iwishart.inverse.chol <- diwishart_inverse(X.inv.chol, df.Press, Sigma.chol,           log  = FALSE, is.chol = TRUE)
dens.iwishart.inverse.arma.chol <- diwishart_inverse_arma(X.inv.chol, df.Press, Sigma.chol, logd = FALSE, is_chol = TRUE)
...
dens <- dmvnorm(x, mu, Sigma, log = FALSE)
ldens <- log(dens)

test_that('dmvnorm_arma is correct: pass full', {
	dens.arma <- dmvnorm_arma(t(x), mu, Sigma, FALSE, FALSE)[1]
	ldens.arma <- dmvnorm_arma(t(x), mu, Sigma, TRUE, FALSE)[1]
	expect_equal(dens, dens.arma)
	expect_equal(ldens, ldens.arma)
})

test_that('dmvnorm_arma is correct: pass Cholesky', {
	dens.arma <- dmvnorm_arma(t(x), mu, Sigma.chol, FALSE, TRUE)[1]
	ldens.arma <- dmvnorm_arma(t(x), mu, Sigma.chol, TRUE, TRUE)[1]
	expect_equal(dens, dens.arma)
	expect_equal(ldens, ldens.arma)
})

# Normal RNG
test_that('rmvnorm_arma is correct: full vs Cholesky', {
	set.seed(seed)
	r.full <- rmvnorm_arma(1, mu, Sigma, is_chol = FALSE)
	set.seed(seed)
	r.chol <- rmvnorm_arma(1, mu, Sigma.chol, is_chol = TRUE)
...
ldetX <- log(det(X))
ldetX.chol <- ldet_from_Cholesky(X.chol)
