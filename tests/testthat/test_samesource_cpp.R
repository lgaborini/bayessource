...
set.seed(seed)
X <- rwish_C(df, Sigma, FALSE, FALSE)
X.inv <- solve(X)
X.chol <- chol(X)
X.inv.chol <- chol(X.inv)


test_that('rwish_C is correct: pass Cholesky, return full', {
	set.seed(seed)
	X.test <- rwish_C(df, Sigma.chol, TRUE, FALSE)
	expect_equal(X, X.test)
	expect_equal(chol(X), chol(X.test))
})

test_that('rwish_C is correct: pass Cholesky, return Cholesky', {
	set.seed(seed)
	X.chol.test <- rwish_C(df, Sigma.chol, TRUE, TRUE)
	expect_equal(X, t(X.chol.test) %*% X.chol.test)
	expect_equal(X.chol, X.chol.test)
})

test_that('rwish_C is correct: pass full, return Cholesky', {
	set.seed(seed)
	X.chol.test <- rwish_C(df, Sigma, FALSE, TRUE)
	expect_equal(X, t(X.chol.test) %*% X.chol.test)
	expect_equal(X.chol, X.chol.test)
})
...
# rwish_C should generate from the same distribution as rWishart does
...
list.rwish_C.samples <- replicate(n.samples, rwish_C(df.Anderson, Sigma), simplify = FALSE)

rWishart.mean <- apply(rWishart.samples, c(1,2), mean)
rwish_C.mean <- apply(simplify2array(list.rwish_C.samples), c(1,2), mean)

# wishart.mean.exact
# rWishart.mean

# stopifnot(all.equal(rWishart.mean, wishart.mean.exact, tolerance = 1e-2))
# stopifnot(all.equal(rwish_C.mean, wishart.mean.exact, tolerance = 1e-2))
...
is.upper <- (rwish_C.mean + z * sd.mc.error) <  wishart.mean.exact
is.lower <- (rwish_C.mean - z * sd.mc.error) >  wishart.mean.exact
test.result <- all(is.upper & is.lower)

test_that(paste('rwish_C converges to the mean, confidence', 1 - alpha),
...
dens.iwishart.arma <- diwishart_inverse_C(X.inv, df.Press, Sigma,  logd = FALSE, is_chol = FALSE)
ldens.iwishart.arma <- diwishart_inverse_C(X.inv, df.Press, Sigma, logd = TRUE,  is_chol = FALSE)
...
# Cholesky passes
ldens.iwishart.arma.chol <- diwishart_inverse_C(X.inv.chol, df.Press, Sigma.chol, logd = TRUE,  is_chol = TRUE)
dens.iwishart.chol <- diwishart_inverse(X.inv.chol, df.Press, Sigma.chol,            log  = FALSE, is.chol = TRUE)
dens.iwishart.arma.chol <- diwishart_inverse_C(X.inv.chol, df.Press, Sigma.chol,  logd = FALSE, is_chol = TRUE)

test_that('R diwishart_inverse (Press) = Rcpp diwishart_inverse_C (Press), pass Cholesky', {
...
test_that('Rcpp diwishart_inverse_C (Press), pass full = Rcpp diwishart_inverse_C (Press), pass Cholesky', {
...
dens.iwishart.inverse.arma <- diwishart_inverse_C(X.inv, df.Press, Sigma, logd = FALSE, is_chol = FALSE)
...
dens.iwishart.inverse.arma.chol <- diwishart_inverse_C(X.inv.chol, df.Press, Sigma.chol, logd = FALSE, is_chol = TRUE)
...
test_that('dmvnorm_C is correct: pass full', {
	dens.arma <- dmvnorm_C(t(x), mu, Sigma, FALSE, FALSE)[1]
	ldens.arma <- dmvnorm_C(t(x), mu, Sigma, TRUE, FALSE)[1]
	expect_equal(dens, dens.arma)
	expect_equal(ldens, ldens.arma)
})

test_that('dmvnorm_C is correct: pass Cholesky', {
	dens.arma <- dmvnorm_C(t(x), mu, Sigma.chol, FALSE, TRUE)[1]
	ldens.arma <- dmvnorm_C(t(x), mu, Sigma.chol, TRUE, TRUE)[1]
	expect_equal(dens, dens.arma)
	expect_equal(ldens, ldens.arma)
})

# Normal RNG
test_that('rmvnorm_C is correct: full vs Cholesky', {
	set.seed(seed)
	r.full <- rmvnorm_C(1, mu, Sigma, is_chol = FALSE)
	set.seed(seed)
	r.chol <- rmvnorm_C(1, mu, Sigma.chol, is_chol = TRUE)
