...
```
Notice that `results` now is a `list`, where `results$LR.num` holds the marginal likelihood value, and `results$mcmc` is the `coda` object which holds the chain output.
