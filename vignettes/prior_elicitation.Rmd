...
n <- 200     # number of items per source
```
...
```{r}
list_exact <- list()
list_exact$B.exact <- randinvmat(p)      # Between Covariance matrix
...
```{r, results='asis'}
bmatrix(solve(list_exact$B.exact), pre = 'B^{-1} =', digits = bmatrix.digits)
```
...
ml <- marginalLikelihood(mtx_data, n.iter = 1000, burn.in = 100,
                   B.inv = priors$B.inv, 
